...
module.exports = {
    FiveStation :async (req,res)=>{   
        const liveStation = await FindLiveStation.findStation(1005,req.params.stationName , req.params.way);  //? 실시간 도착모듈

        if(liveStation.suc){ //? 열차정보가 있다면 복잡도 생성
            const BokJobDo = await MakeBokJobDo(5 , liveStation.data.trainNumber); //? 복잡도 생성
